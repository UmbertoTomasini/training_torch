...
class LlaMA:
    ...
    def text_completion(self,prompts: List[str], temperature: float = 0.6, top_p: float = 0.9, max_gen_len: Optional[int]=None):
        ...
        #make sure do not excees max batch size
        batch_size = len(prompt_tokens)
        assert batch_size <=self.args.max_batch_size

        max_prompt_len = max(len(prompt) for prompt in prompts)
        # make sure prompt lenght not large than max seq len
        assert max_prompt_len <= self.args.max_seq_len

        total_len = min(self.args.max_seq_len, max_gen_len + max_prompt_len)

        #create list that will contrain the generated token, along with initial prompt tokens
        ...
        for cur_pos in tqdm(range(1,total_len),desc="Generating tokens"):
            ...
            # we replace the token ONLY if it is a padding token    
    ...
    def _sample_top_p(self,probs,p):
        ...
        probs_sort[mask] = 0
        probs_sort.div_(probs_sort.sum(dim=-1,keepdim=True)) #redistribute probs

        next_tokens = torch.multinomial(probs_sort,num_samples=1) #indicates index in the SORTED probs
...
if __name__ == "__main__":
    ...
    model = LlaMA.build(
        checkpoint_dir= "llama2_models/llama-2-7b/",
        tokenizer_path='llama2_models/tokenizer.model',
        load_model=True,
        max_seq_len=1024,
